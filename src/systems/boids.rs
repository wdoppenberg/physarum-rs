...
pub fn update_boids(mut boids: Query<&mut Boid>, config: Res<PhysarumConfig>, time: Res<Time>) {
...
pub fn spawn_boid_at(commands: &mut Commands, x: f32, y: f32, direction: Vec2) {
    ...
    commands.spawn(boid);
    debug!(
        "Spawned boid at ({x}, {y}) with direction ({}, {})",
...
pub fn spawn_random_boid(mut commands: Commands, config: Res<PhysarumConfig>, time: Res<Time>) {
